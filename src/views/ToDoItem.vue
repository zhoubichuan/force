...
export default {
  mounted() {
    ...
    function render(width, height, nodes, links) {
      ...
      //定义缩放事件
      let zoom = d3.behavior.zoom().scaleExtent([1, 2]).on("zoom", zoomed)
      ...
      //定义拖拽事件
      let drag = graph.force.drag().on("dragstart", (d) => {
      ...
      graph.feGaussianBlur = graph.defs.append("feGaussianBlur").attr({
        stdDeviation: 1,
      })

      graph.feGaussianBlur.append("animate").attr({
        id: "filter-anim-in",
      ...
      //(3)设置线条上的文字
      graph.linesText = graph.container
        .append("g")
        .selectAll(".edgelabel")
        .data(graph.force.links())
        .enter()
      ...
      //(4)圆圈节点内文字
      graph.nodesText = graph.container
        .append("g")
        .selectAll("text")
        .data(graph.force.nodes())
        .enter()
